...
/**
 * A business activity, typed straight in SQL: the catalog service of this
 * branch still creates personal ones, and its extension is another issue.
 */
async function businessActivity(user: string, name = 'Freelance'): Promise<string> {
  const activity = await createActivity(user, name)
  await db()`update activity set kind = 'business', started_on = '2026-01-01' where id = ${activity.id}`
  return activity.id
}
...
test('a rule refuses what would not compute, and says which field to fix', async () => {
  const user = await seedUser()
  const activity = await businessActivity(user)
  const personal = await createActivity(user, 'Perso')
