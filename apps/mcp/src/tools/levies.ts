    ...
    'manage_levies',
    {
      ...
      inputSchema: z.object({
        ...
        elective: electiveSchema
          ...
          .optional()
          .describe(
            "amountForm elective_base: {rows: [{upTo, minBase, maxBase}], inputName, rate}. The reference (the base block) falls in a row, and the user's chosen base, stored as the dated input inputName, is what the amount is rate % of. Without a regularization the choice is held inside that row's bounds; with annual_deadzone it stands as declared and the closed year's bounds are confronted once, by the regularization",
          ),
