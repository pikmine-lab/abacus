...
/**
 * The path itself, wherever a screen is empty for a reason someone can act on:
 * the first run, and an activity that carries no rule yet. Only the next open
 * step calls to action, so the way forward is one thing and not a menu.
 */
export function StepPath({ steps }: { steps: Step[] }) {
    ...
    <ol className="flex flex-col">
      {steps.map((step, i) => {
        ...
        return (
          <li
            key={step.href}
            className={cn('flex gap-3 border-t border-border py-4 last:border-b', step.done && 'opacity-55')}
          >
            <span
              className={cn(
